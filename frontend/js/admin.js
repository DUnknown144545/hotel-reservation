    ...
    async function loadDashboardStats() {
      try {
        ...
        if (data && data.success && data.stats) {
          document.getElementById('stat-total-rooms').textContent = data.stats.total_rooms ?? 0;
          document.getElementById('stat-occupied').textContent = data.stats.occupied_rooms ?? 0;
          document.getElementById('stat-guests').textContent = data.stats.current_guests ?? 0;
          document.getElementById('stat-revenue').textContent = '₱ ' + (parseFloat(data.stats.monthly_revenue) || 0).toFixed(2);
        }
      } catch (error) {
    ...
    async function loadUsers() {
      ...
      try {
        ...
        if (data && data.success) {
          ...
          container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-exclamation-circle"></i><p>Error loading users</p></div>';
        }
      } catch (error) {
        console.error('Error loading users:', error);
        container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-exclamation-circle"></i><p>Network error</p></div>';
      }
    }
    ...
    function displayUsers(users) {
      ...
      if (!users || users.length === 0) {
        container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-users"></i><p>No users found</p></div>';
        return;
      }
      ...
      users.forEach(u => {
        ...
        const registeredDate = u.created_at ? new Date(u.created_at).toLocaleDateString() : '-';
    ...
    function displayRooms(rooms) {
      ...
      if (!rooms || rooms.length === 0) {
        container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-bed"></i><p>No rooms found</p></div>';
        return;
      }
      ...
      rooms.forEach(room => {
        ...
        const statusClass = status === 'Available' ? 'status-accepted' : status === 'Occupied' ? 'status-pending' : 'status-declined';
        ...
        html += `
          <tr>
            ...
            <td>
              ...
              <button class="action-btn btn-decline" onclick="deleteRoom(${room.id}, '${room.room_number}')">
    ...
    function editRoom(room) {
      ...
      document.getElementById('roomImage').value = room.image_data || room.image_url || '';
      
      if (room.image_data || room.image_url) {
        document.getElementById('roomImagePreview').src = room.image_data || room.image_url;
    ...
    async function saveRoom(event) {
      ...
      try {
        ...
        const data = await apiFetch(url, { method, body: JSON.stringify(roomData) });
        if (data && data.success) {
          alert(data.message);
          closeRoomModal();
          loadRooms();
    ...
    async function deleteRoom(roomId, roomNumber) {
      ...
      try {
        const data = await apiFetch(`${API_URL}/rooms/${roomId}`, { method: 'DELETE' });
        if (data && data.success) {
          alert(data.message);
          loadRooms();
          loadDashboardStats();
    ...
    async function loadAllBookings() {
      ...
      try {
        ...
        if (data && data.success) {
          ...
          container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-calendar-xmark"></i><p>Error loading bookings</p></div>';
        }
      } catch (error) {
    ...
    function displayAllBookings(bookings) {
      ...
      if (!bookings || bookings.length === 0) {
        container.innerHTML = '<div class="empty-state"><i class="fa-solid fa-calendar"></i><p>No bookings found</p></div>';
        return;
      }
      ...
      bookings.forEach(booking => {
        ...
        html += `
          <tr>
            ...
            <td>${booking.room_number || 'N/A'} (${booking.room_type || '-'})</td>
            <td>${booking.checkin_date ? new Date(booking.checkin_date).toLocaleDateString() : '-'}</td>
            <td>${booking.checkout_date ? new Date(booking.checkout_date).toLocaleDateString() : '-'}</td>
            <td><span class="status-badge ${paymentStatus === 'Paid' ? 'status-accepted' : 'status-pending'}">${paymentStatus}</span></td>
            <td><span class="status-badge ${status === 'Checked In' ? 'status-accepted' : status === 'Pending' ? 'status-pending' : 'status-declined'}">${status}</span></td>
          </tr>
        `;
      });
    ...
    async function loadReports() {
      try {
        ...
        if (bookingsData && bookingsData.success) {
          document.getElementById('report-bookings').textContent = bookingsData.bookings.length;
        }

        if (usersData && usersData.success) {
          document.getElementById('report-users').textContent = usersData.users.length;
        }
        ...
        if (statsData && statsData.success) {
          ...
          document.getElementById('report-occupancy').textContent = occupancyRate + '%';
        }
